...
func TestIntegration_RebaseWorktrees(t *testing.T) {
	remoteDir := filepath.Join(t.TempDir(), "remote.git")
	cloneDir := filepath.Join(t.TempDir(), "clone")
	...
	issue250Git(t, cloneDir, "commit", "-m", "base")
	issue250Git(t, cloneDir, "push", "-u", "origin", "main")
	baseSHA := issue250Git(t, cloneDir, "rev-parse", "main")

	issue250Git(t, cloneDir, "checkout", "-b", "b1")
	issue250WriteFile(t, cloneDir, "b1.txt", "b1\n")
	issue250Git(t, cloneDir, "add", ".")
	issue250Git(t, cloneDir, "commit", "-m", "b1")
	b1SHA := issue250Git(t, cloneDir, "rev-parse", "b1")
	issue250Git(t, cloneDir, "checkout", "-b", "b2")
	issue250WriteFile(t, cloneDir, "b2.txt", "b2\n")
	issue250Git(t, cloneDir, "add", ".")
	issue250Git(t, cloneDir, "commit", "-m", "b2")
	b2SHA := issue250Git(t, cloneDir, "rev-parse", "b2")
	issue250Git(t, cloneDir, "checkout", "-b", "b3")
	issue250WriteFile(t, cloneDir, "b3.txt", "b3\n")
	issue250Git(t, cloneDir, "add", ".")
	issue250Git(t, cloneDir, "commit", "-m", "b3")
	b3SHA := issue250Git(t, cloneDir, "rev-parse", "b3")

	writeStackFile(t, filepath.Join(cloneDir, ".git"), stack.Stack{
		Trunk: stack.BranchRef{Branch: "main", Head: baseSHA},
		Branches: []stack.BranchRef{
			{Branch: "b1", Head: b1SHA, Base: baseSHA},
			{Branch: "b2", Head: b2SHA, Base: b1SHA},
			{Branch: "b3", Head: b3SHA, Base: b2SHA},
		},
	})
	issue250Git(t, cloneDir, "checkout", "main")
	issue250Git(t, cloneDir, "worktree", "add", b2Dir, "b2")
	issue250Git(t, cloneDir, "worktree", "add", b3Dir, "b3")
	issue250Git(t, cloneDir, "config", "rebase.updateRefs", "true")

	issue250Git(t, ".", "clone", remoteDir, upstreamDir)
	...
	require.ErrorIs(t, runRebase(cfg, &rebaseOptions{noTrunk: true}), ErrSilent)
	cfg.Err.Close()
	guidance, _ := io.ReadAll(errR)
	assert.Contains(t, string(guidance), "--worktrees")

	cfg = issue250TestConfig(t)
	require.NoError(t, runRebase(cfg, &rebaseOptions{remote: "origin", worktrees: true}))
	for _, pair := range [][2]string{{"main", "b1"}, {"b1", "b2"}, {"b2", "b3"}} {
		require.NoError(t, issue250GitMayFail(t, cloneDir, "merge-base", "--is-ancestor", pair[0], pair[1]))
	}
	assert.Equal(t, "main", issue250Git(t, cloneDir, "branch", "--show-current"), "the unowned first branch is restored after checkout-based rebase")
	assert.Equal(t, "b2", issue250Git(t, b2Dir, "branch", "--show-current"))
	assert.Equal(t, "b3", issue250Git(t, b3Dir, "branch", "--show-current"))
}

func TestIntegration_RebaseWorktreesConflictCanContinueAndAbort(t *testing.T) {
	remoteDir := filepath.Join(t.TempDir(), "remote.git")
	cloneDir := filepath.Join(t.TempDir(), "clone")
	...
	issue250Git(t, cloneDir, "config", "user.name", "Test")
	issue250Git(t, cloneDir, "config", "user.email", "test@example.com")
	issue250WriteFile(t, cloneDir, "shared.txt", "base\n")
	issue250Git(t, cloneDir, "add", ".")
	issue250Git(t, cloneDir, "commit", "-m", "base")
	issue250Git(t, cloneDir, "push", "-u", "origin", "main")
	base := issue250Git(t, cloneDir, "rev-parse", "main")
	issue250Git(t, cloneDir, "checkout", "-b", "b1")
	issue250WriteFile(t, cloneDir, "b1.txt", "b1\n")
	issue250Git(t, cloneDir, "add", ".")
	issue250Git(t, cloneDir, "commit", "-m", "b1")
	b1 := issue250Git(t, cloneDir, "rev-parse", "b1")
	issue250Git(t, cloneDir, "checkout", "-b", "b2")
	issue250WriteFile(t, cloneDir, "shared.txt", "b2\n")
	issue250Git(t, cloneDir, "add", ".")
	issue250Git(t, cloneDir, "commit", "-m", "b2")
	b2 := issue250Git(t, cloneDir, "rev-parse", "b2")
	issue250Git(t, cloneDir, "checkout", "-b", "b3")
	issue250WriteFile(t, cloneDir, "b3.txt", "b3\n")
	issue250Git(t, cloneDir, "add", "b3.txt")
	issue250Git(t, cloneDir, "commit", "-m", "b3")
	b3 := issue250Git(t, cloneDir, "rev-parse", "b3")
	writeStackFile(t, filepath.Join(cloneDir, ".git"), stack.Stack{Trunk: stack.BranchRef{Branch: "main", Head: base}, Branches: []stack.BranchRef{{Branch: "b1", Head: b1, Base: base}, {Branch: "b2", Head: b2, Base: b1}, {Branch: "b3", Head: b3, Base: b2}}})
	issue250Git(t, cloneDir, "checkout", "main")
	issue250Git(t, cloneDir, "worktree", "add", b1Dir, "b1")
	issue250Git(t, cloneDir, "worktree", "add", b3Dir, "b3")
	issue250WriteFile(t, b1Dir, "shared.txt", "b1 amended\n")
	issue250Git(t, b1Dir, "add", "shared.txt")
	issue250Git(t, b1Dir, "commit", "--amend", "-m", "b1 amended")

	withIssue250Repo(t, cloneDir)
	cfg := issue250TestConfig(t)
	err := runRebase(cfg, &rebaseOptions{worktrees: true, noTrunk: true})
	require.ErrorIs(t, err, ErrConflict)
	assert.True(t, git.IsRebaseInProgressAt(cloneDir))

	issue250WriteFile(t, cloneDir, "shared.txt", "resolved\n")
	issue250Git(t, cloneDir, "add", "shared.txt")
	issue250WriteFile(t, b3Dir, "dirty.txt", "dirty\n")
	withIssue250Repo(t, b1Dir)
	err = runRebase(cfg, &rebaseOptions{cont: true})
	require.Error(t, err)
	assert.True(t, git.IsRebaseInProgressAt(cloneDir))
	_, err = os.Stat(filepath.Join(cloneDir, ".git", rebaseStateFile))
	assert.NoError(t, err)
	require.NoError(t, os.Remove(filepath.Join(b3Dir, "dirty.txt")))
	require.NoError(t, runRebase(cfg, &rebaseOptions{cont: true}))
	assert.False(t, git.IsRebaseInProgressAt(cloneDir))
	require.NoError(t, issue250GitMayFail(t, cloneDir, "merge-base", "--is-ancestor", "b1", "b2"))
	require.NoError(t, issue250GitMayFail(t, cloneDir, "merge-base", "--is-ancestor", "b2", "b3"))

	// Start a second conflict and abort it from the main worktree.
	issue250WriteFile(t, b1Dir, "shared.txt", "b1 amended again\n")
	issue250Git(t, b1Dir, "add", "shared.txt")
	issue250Git(t, b1Dir, "commit", "--amend", "-m", "b1 amended again")
	b1BeforeAbort := issue250Git(t, cloneDir, "rev-parse", "b1")
	issue250Git(t, cloneDir, "reset", "--hard", b2)
	withIssue250Repo(t, cloneDir)
	err = runRebase(cfg, &rebaseOptions{worktrees: true, noTrunk: true})
	require.ErrorIs(t, err, ErrConflict)
	withIssue250Repo(t, b1Dir)
	...
	assert.False(t, git.IsRebaseInProgressAt(cloneDir))
	assert.Equal(t, b1BeforeAbort, issue250Git(t, cloneDir, "rev-parse", "b1"))
	assert.Equal(t, b2, issue250Git(t, cloneDir, "rev-parse", "b2"))
}
